/****** program lab_14, sprawozdanie nr 14 na jego podstawie ******/
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
...
int main()
{
	...
	if(!file_U) 
		printf("Error during creating a file: 'U.dat' !");
	FILE *file_U_hist = fopen("U_hist.dat", "w");
	if(!file_U_hist) 
		printf("Error during creating a file: 'U_hist.dat' !");
	FILE *file_T_hist = fopen("T_hist.dat", "w");
	if(!file_T_hist) 
		printf("Error during creating a file: 'T_hist.dat' !");
...
///////////////////////////////////////////////////// ROZKLAD TROJKATNY /////////////////////////////////////////////////////
	...
	for (int i = 0; i < N; i++)
	{	
		ksi_1 = gen_1( a, c, m_p);		// a = 69069, c = 1, m = 2^32 
		ksi_2 = gen_1( a, c, m_p);
	...
	for (int j = 0; j < K; j++)
	{
		// teoretyczne prawdopodobienstwo
		pj[j] = F( ((mi - d) + ( j + 1 ) * dx), mi, d )  -  F( ((mi - d) + j * dx), mi, d );
		
		printf("n[%d] = %d\n", j, nj3[j]);
		fprintf(file_T_hist, "%f\t%f\t%f\n", (mi - d)+(j + 0.5) * dx, (double)nj3[j] / N, pj[j]);
	}


	
/*************************** test chi^2 ***************************/
	double chi2 = 0.0;	// w tym przypadku wartość tablicowa eps = 14.06
